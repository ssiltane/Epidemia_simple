...
t1 = text(.35*Nt,1.1*Nbeds,'Number of ICU beds');
...
for ppp = 1:Nepidemics
    ...
    axis([1 Nt 0 N])
    t2 = text(.1*Nt,17/25*N,['A person meets ',num2str(Nmeet),' others daily']);
...
for ppp = 1:Nepidemics
    ...
    axis([1 Nt 0 N])
    t3 = text(.5*Nt,4.5/25*N,['A person meets ',num2str(Nmeet),' others daily']);
    set(t3,'fontsize',fsize,'color',[0 0 1])
    title('Simpel model for infectious disease spread','fontsize',fsize)
    drawnow
end
